...
def add_review(request, teacher_id):
    ...
    if not Lesson.has_completed_lesson(student=request.user, teacher=teacher):
        return render(
            request,
            "catalog2/error.html",
            {"message": "Вы можете оставить отзыв только после завершенного занятия."},
        )
    ...
    return render(
        request, "catalog2/add_review.html", {"form": form, "teacher": teacher}
    )
